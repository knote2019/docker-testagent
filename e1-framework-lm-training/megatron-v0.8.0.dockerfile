...
# install flash-attn.
RUN set -x \
&& git clone -b v2.6.3 --recursive --depth=1 https://github.com/Dao-AILab/flash-attention.git /tmp/flash-attention \
...
RUN set -x \
&& git clone -b v2.1 --recursive --depth=1 https://github.com/NVIDIA/TransformerEngine.git /tmp/TransformerEngine \
...
RUN set -x \
&& pip install nltk \
&& git clone -b core_v0.8.0 --recursive --depth=1 https://github.com/NVIDIA/Megatron-LM.git /tmp/megatron \
&& cd /tmp/megatron \
&& python setup.py install \
&& rm -rf /tmp/* \
&& echo "end"
