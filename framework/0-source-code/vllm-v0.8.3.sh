...
git checkout main
git branch -D master
git checkout v0.8.3 -b master
git remote set-url origin ssh://git@bitbucket.iluvatar.ai:7999/swte/vllm-v0.8.3.git
...
rm -rf requirements/rocm-build.txt
rm -rf requirements/rocm-test.txt
