...
# install cuda.
RUN set -x \
&& echo 'deb http://archive.ubuntu.com/ubuntu focal main universe' > /etc/apt/sources.list.d/tmp.list \
&& apt update \
&& apt install -y gcc-7 \
&& apt install -y g++-7 \
...
&& wget -nv http://10.113.3.1/corex/toolbox/cuda/cuda_10.2.89_440.33.01_linux.run -P /tmp \
&& bash /tmp/cuda_10.2.89_440.33.01_linux.run --toolkit --silent \
&& wget -nv http://10.113.3.1/corex/toolbox/cuda/cudnn-10.2-linux-x64-v7.6.5.32.tgz -P /tmp \
