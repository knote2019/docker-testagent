...
# install flash-attn.
RUN set -x \
&& git clone -b v2.4.2 --recursive --depth=1 https://github.com/Dao-AILab/flash-attention.git /usr/local/flash-attention \
...
RUN set -x \
&& git clone -b v1.6 --recursive --depth=1 https://github.com/NVIDIA/TransformerEngine.git /usr/local/TransformerEngine \
