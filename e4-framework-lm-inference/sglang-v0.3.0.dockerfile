...
# install vllm.
RUN set -x \
&& git clone -b v0.2.0 --recursive https://github.com/vllm-project/vllm.git /usr/local/vllm \
&& pip install -e /usr/local/vllm --no-build-isolation --verbose \
&& echo "end"
...
&& pip install -e /usr/local/sglang/python --no-build-isolation --verbose \
&& echo "end"
